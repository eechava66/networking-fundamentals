...
class Host(Device):
    ...
    def arp(self):
        ...
        for device, port in self.connected_devices:
            print(
                f"Sending ARP from: {self.device_name} to: {device.device_name} trough port: {port}"
            )
            device.messages = [*device.messages, {**frame, "port": port}]
        ...
        if destination_ip in self.routing_table.keys():
            ...
            for device, port in self.connected_devices:
                if isinstance(device, Switch):
                    ...
                    device.messages = [*device.messages, {**packet, "port": port}]
                    return
        else:
            self.arp()
